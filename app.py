...
def index():
    ...
    for n in os.listdir(path_json):    # serch files name
        with open(path_json + '/' + n) as f:    # open the files
            ...
            news_dict[n.split('.')[0]] = [filememo.get('title'), filememo.get('created_time'), filememo.get('content')]
...
def files(filename):
    if filename+'.json' in os.listdir(path_json):
        ...
        return render_template('file.html', filename=filename, title=news_title, ct=news_ct, content=news_content)
    else:
