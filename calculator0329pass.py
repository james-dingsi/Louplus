...
if __name__ == '__main__':
    ...
    with open(readfiles.paylist(), 'w', encoding='utf-8') as pl:
        output_list = []
        for i, l in output_dict.items():
            l.insert(0, int(i))
            l.append(time.strftime('%Y-%m-%d', time.localtime(time.time())))
            l.append(time.strftime('%H:%M:%S', time.localtime(time.time())))
            output_list.append(l)
#        print(output_list)
        csv.writer(pl).writerows(output_list)
