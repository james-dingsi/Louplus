...
class LiveForm(FlaskForm):
	name = StringField('直播名称', validators=[DataRequired(), Length(3, 24)])
	live_url = StringField('直播地址', validators=[DataRequired(), URL()])
	user_id = IntegerField('直播用户ID', validators=[DataRequired(), NumberRange(min=1, message='直播用户不存在')])
	submit = SubmitField('提交')

	def validate_user_id(self, field):
		if not User.query.get(field.data):
			raise ValidationError('直播用户不存在')
